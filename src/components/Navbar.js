...
class Navbar extends React.Component {
  ...
  render() {
    ...
    const navbarHorizontal = navbarItems.map((navbarItem) => (
      <h5
        className="navbar-purple-heading"
    ...
    const navbarVertical = navbarItems.map((navbarItem) => (
      <h3 key={navbarItem.id}>
        <Link
          to={navbarItem.id}
          spy={true}
          smooth={true}
    ...
    return (
      <div
        ...
        style={{ transition: "1s ease" }}
        id="navbar-sticky"
        onScroll={this.setNavBarColor}
      >
        <div id="navbar-container">
          <div id="img-container">
            <img
              className="navbar-wic-logo"
              alt="wic-logo"
              src={wic_purple}
          ...
          {this.props.windowWidth > 800 ? (
            ...
            <>
              <SlidingPanel
                ...
                type={"right"}
                size={this.props.windowWidth > 576 ? 45 : 80}
                hideHeader
                backdropClicked={this.toggleButton}
              >
                <div id="vertical-navbar-container">{navbarVertical}</div>
              </SlidingPanel>
              <HamburgerCollapse
                buttonWidth={25}
                isActive={this.state.isActive}
